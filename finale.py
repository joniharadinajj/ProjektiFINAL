...
label = tk.Label(root,text="COUNTDOWN" , font=("Times New Roman",20), fg="#003049")
label.pack(padx=20, pady=20)
label.config(bg="#caf0f8", font=("Times New Roman",20, ""))

label = tk.Label(root,text="Use COUNTDOWN for more productive work time" , font=("Times New Roman",14), fg="#003049")
label.pack(padx=10, pady=25)
label.config(bg="#caf0f8")
# variablat
hour = StringVar()
minute = StringVar()
second = StringVar()
...
def on_hover(event):
    event.widget.configure(bg="#e9ecef" , fg="#003049")
...
# button widget
def on_hover(event):
    event.widget.configure(bg="#003049", fg="white")
def on_default(event):
    event.widget.configure(bg="#669bbc")
btn = Button(root, text='Start COUNTDOWN Timer', bd='1', width=22, height=2, font=("Times New Roman", 10),
             command=submit)
btn.place(x=168, y=330)
btn.bind("<Enter>", on_hover)
btn.bind("<Leave>", on_default)

label= tk.Label(root, text="Joni Haradinaj      /       jCoders", font=("Arial", 10), fg="#003049")
label.place(x=145 ,y=450)
label.config(bg="#caf0f8")
